...
my @authors;
my %affiliations;
my ( $article_c, $pmid_c, $pmcid_c, $title_c, $year_c, $text_c ) =
  ( 0, 0, 0, 0, 0, 0 );

# =========================== Main
$keyword = lc $keyword;    # comment this line to do case sensitive search

open PAPER, ">$paper_file_name"
  or die "can't open paper TSV file $paper_file_name to write";
open AUTHOR, ">$author_file_name"
  or die "can't open author TSV file $author_file_name to write";
...
while (<>) {
    ...
    if ( /<article-id pub-id-type=\"pmc\">(\d+)<\/article-id>/
        && length($pmcid) == 0 )
    {
        $pmcid = $1;
    ...
    if (/<contrib .*contrib-type=\"author\"/) {
        ...
        while ( $_ !~ /<\/contrib>/ ) {
            ...
            $a[2] = $1
              if /<xref ref-type=\"aff\" rid=\"(\S+?)(\s|\")/
              && !defined( $a[2] );
            $a[2] = $1
              if /<xref rid=\"(\S+?)(\s|\") ref-type=\"aff\"/
              && !defined( $a[2] );
            $_ = <>;
        }
    ...
    if (/<aff id=\".+?\"/) {
        ...
        $a =~ /<aff id=\"(.+?)\".*?>(.+)<\/aff>/
          or die "unexpected pattern match errors";
        ( $i, $a ) = ( $1, $2 );
        $a =~ s/^\s*(<([^>]+)>.{0,10}<\/\2>|<[^>]+\/>)\s*//;
        $a =~ s/<email>.*<\/email>//g
          ;    # uncomment this line if you want email addresses removed
        $a =~ s/<.+?>//g
          ;    # uncomment this line if you want HTML markups removed, too
        $affiliations{$i} = $a;
    }
    if (/<aff>(.+?)<\/aff>/) {
        $shared_aff = $1;
    }
    if ( /<pub-date .*?(pub-type=\"[pe]pub\"|date-type=\"pub\")/
        && length($year) == 0 )
    {
        while (1) {
            if (/<year.*?>(\d+)<\/year/) {
    ...
    if ( /<\/body>/ && defined($text) ) {
        ...
        $count -= () =
          $copy =~ /\.$keyword\./g;    # uncount cases like www.cytoscape.org
        if ( $count > 0 ) {
            if ($pmcid) {
                $title =~ s/<italic>//g;
                $title =~ s/<\/italic>//g;
                ...
                for (@authors) {
                    ...
                    if ( defined( $_->[2] ) ) {
                        if ( defined( $affiliations{ $_->[2] } ) ) {
                            ...
                            warn
"missing affiliation info $_->[2] mentioned on author list for PMC $pmcid";
                        }
                    }
                    elsif ($shared_aff) {
...
close AUTHOR;
print STDERR
"Scanned $article_c articles, $pmcid_c PMC ids, $title_c titles, $year_c years and $text_c text bodies\n";
